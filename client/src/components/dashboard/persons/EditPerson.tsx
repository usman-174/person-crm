...
export function EditPerson({ person }: props) {
  ...
  const deleteSocialMutation = useMutation({
    ...
    onSuccess: async () => {
      let { data } = await axios.post("/api/revalidate", {
        tags: [...REAVALIDAION_TIME.USER.TAGS(person.id)],
      });
    ...
    onSuccess: async () => {
      ...
      let { data } = await axios.post("/api/revalidate", {
        tags: [
          ...
          ...REAVALIDAION_TIME.USER.TAGS(person.id),
        ],
      });
      if (data) {
  ...
  function onSubmit(values: z.infer<typeof editPersonSchema>) {
    console.log({ social: values.social! });
            ...
            {person?.social?.length &&
              person.social!.map((social, ind) => (
                ...
                >
                  ...
                  <Button
                    type="button"
                    className="mt-7 bg-destructive"
                    size={"sm"}
                    onClick={() => {
                      confirm("Are you sure you want to delete this social?") &&
